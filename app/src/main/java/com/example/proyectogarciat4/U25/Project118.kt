package com.example.proyectogarciat4.U25
...
fun Project118() {
    ...
    var ages by remember { mutableStateOf("") }
...
class Kids() {
    var age: IntArray = IntArray(5)

    fun load(ageStr : String){
        var text = ""
        val ageEnter = ageStr.split(",").mapNotNull { it.toIntOrNull() }.toIntArray()
        if(ageEnter.size == 5){
            age = ageEnter
        }else{
            text += "Enter 5 age separated by comas"
        }
    }

    fun higherAge(): Int{
        return age.maxOrNull() ?: 0
    }
